...
const oddValues = (arr) => {
  let q1answer = arr.filter( (value) => value%2 === 1);
  return q1answer;
};
...
const filterStringsWithVowels = (arr) => {
  const q2answer = arr.filter( (value) => value.match(/[aeiou]/g));
  return q2answer;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let q3Answers = arr.filter( (value) => forbiddenValues.indexOf(value) === -1);
  return q3Answers;
};
