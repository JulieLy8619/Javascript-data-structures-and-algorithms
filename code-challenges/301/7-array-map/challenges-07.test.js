...
const mapTwoToThe = (arr) => {
  let mapAnaswer = arr.map( (num) => Math.pow(2,num));
  return mapAnaswer;
}
...
const charCode = (arr) => {
  ...
  for (let q = 0; q < arr.length; q++) {
    lettersAnswer.push(arr[q].charCodeAt(0));
  }
...
const evenOdd = (arr) => {
  ...
  arr.map( (num) => {
    // if(num%2 === 0) {
    returnAnswer.push(isNaN(num) ? 'N/A' : (num%2 === 0 ? 'even' : 'odd'))
    // }
  })
  return returnAnswer;
};
...
const extractAbilities = (arr) => {
  let answerArr = [];
  arr.map( (val) => {
    answerArr.push(val.ability.name);
  })
  return answerArr;
};
...
const extractStats = (arr) => {
  ...
  for (let t = 0; t < arr.length; t++) {
    ...
    tempObj.name = '';
    tempObj.total = 0;
    tempObj.name = arr[t].stat.name;
    tempObj.total = arr[t].effort + arr[t].baseStat;
    answerArr.push(tempObj);
  }
  return answerArr;
};
