...
const returnNames = (arr) => {
  const tempArray = [];
  let nameCallback = (total, currentValue, currentIndex, arr) => {
    tempArray.push(arr[currentIndex].name);
  };
  arr.reduce(nameCallback,0);
  return tempArray;
};
...
const countNumberOfChildren = (arr) => {
  ...
  //   if (arr[currentIndex].children !== undefined) {
  //     total + arr[currentIndex].children.length;
  //   }
  // }),0);
  return arr.reduce( ((total, currentValue, currentIndex, arr) => total + arr[currentIndex].children),0);
