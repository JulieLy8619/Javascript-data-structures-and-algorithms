...
const getHouses = (arr) => {
  //was I supposed to write this without knowing the keys, like do the objects.keys and then do a for loop through that array
  let houses = [];
...
const hasChildrenValues = (arr, character) => {
  let answer;
  Object.values(arr).forEach( childObj => {
    if (childObj.name === character) {
      answer = childObj.children.length > 0 ? true : false
    }
  })
  return answer;
};
