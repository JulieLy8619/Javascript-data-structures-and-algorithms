...
//come back to re-write with filter instead of for loop
...
//come back to re-write with I think map or for each and I def could use the reduce
const totalSum = (input) => {
  // return input.forEach( (val, idx, arr) => {
  ...
  for (let i = 0; i< input.length; i++) {
    if (input[i].length === 1) { 
      ...
      for (let t = 0; t < input[i].length; t++){
        tempCounter = tempCounter + input[i][t];
      }
    }
  }
  return tempCounter;
