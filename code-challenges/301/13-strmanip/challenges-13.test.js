...
//not solved, come back for this one
const standardizePhoneNumbers = (arr) => {
  let q3Answer = '';
  for (let i = 0; i < arr.length; i++) {
    for (let j = 0; j < arr[i].length; j++) {
      if (arr[j][i].match(/\d/g)) {
        q3Answer = q3Answer + arr[j][i];
      }
    }
  }
  return q3Answer;
...
const allHappy = (arr) => {
  if (arr.length === (findHappiness(arr).length)) {
    return true;
  } else {
    return false;
  }
...
//this didn't work because there wasn't an escape character for the ) in the :)
const findAnything = (arr, target) => {
  let q6Arr = [];
  let regexstr = `/(${target})/g`
  arr.forEach( (val) => {
    if (val.search(regexstr) > 0) {
      q6Arr.push(val);
    }
  })
  return q6Arr;
